...
# Simulate Voigt Signal

def voigt_signal(a, p0, g, l, n):
    v0 = pk.voigt(0, g, l)
    return [a * pk.voigt(n - p0, g, l) / v0  for n in range(0, n)]

def poly_signal(args, n):
    return [pk.poly(el, *args) for el in range(n)]
...
# Fit Signal
x = range(n)
fit, err, snr, fun, yf = pk.fit_voigt(x, y, peak_ratio = 1 / 5, background = 1, log = 1)
